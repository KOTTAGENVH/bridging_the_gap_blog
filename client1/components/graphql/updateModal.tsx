...
import { update_user_mutation } from '@/app/apollo/queries_Mutations';
...
function UpdateModal({ user, onClose }: UpdateModalProps) {
  const [name, setName] = useState(user.name || '');
  const [username, setUsername] = useState(user.username || '');
  const [age, setAge] = useState(user.age || 0);
  const [nationality, setNationality] = useState<Nationality>(user.nationality || Nationality.US);
